...
def run_agent(client, agent_id, prompt, model="gpt-4o"):
...
End with a **Success Metrics Dashboard** (what good looks like each week).""")],
            ...
            for stage_idx, stage in enumerate(PIPELINE):
                ...
                for agent_id, prompt in stage:
                    run_agent(client, agent_id, prompt, model=gpt_model)
                    completed += 1
...
Keep it concise, motivating, and actionable."""
            ...
            st.session_state.final_plan = synth_resp.choices[0].message.content.strip()
